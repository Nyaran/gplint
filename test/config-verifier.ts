...
describe('Config Verifier', function() {
  describe('Verification is successful when', function() {
    it('rules are set to off/0/warn/1/error/on/2', async function() {
      assert.deepEqual(await verifyConfig.verifyConfigurationFile({
    ...
    it('a rule config is an array of size 2, with an "on/off" state and another config value', async function() {
      assert.deepEqual(await verifyConfig.verifyConfigurationFile({'new-line-at-eof': ['error', 'yes']}), []);
    });

    it('a rule config is an array of size 2, with an "on/off" state and a keyword based array', async function() {
      assert.deepEqual(await verifyConfig.verifyConfigurationFile({
  ...
  describe('Verification fails when', function() {
    it('a non existing rule', async function() {
      assert.deepEqual(await verifyConfig.verifyConfigurationFile({'fake-rule': 'error'}), ['Rule "fake-rule" does not exist']);
    });

    it('a non existing rule sub-config', async function() {
      assert.deepEqual(await verifyConfig.verifyConfigurationFile({
    ...
    it('rule config that\'s not properly configured to "on/off"', async function() {
      assert.deepEqual(await verifyConfig.verifyConfigurationFile({
    ...
    it('an array configuration doesn\'t have exactly 2 parts', async function() {
      assert.deepEqual(await verifyConfig.verifyConfigurationFile({'new-line-at-eof': ['error']}), [
        'Invalid rule configuration for "new-line-at-eof" - The config should only have 2 parts.'
      ]);

      assert.deepEqual(await verifyConfig.verifyConfigurationFile({'new-line-at-eof': ['error', 'yes', 'p3']}), [
        'Invalid rule configuration for "new-line-at-eof" - The config should only have 2 parts.'
      ]);
    });
  });
